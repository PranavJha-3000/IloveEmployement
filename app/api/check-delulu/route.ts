...
export async function POST(req: NextRequest) {
  ...
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. Paste the JD to compare against." },
      { status: 400 }
    );
  }
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildDeluluPrompt({
    resumeText,
    jobDescription,
  ...
  try {
    ...
    const safe = validateDeluluCheckResult(parsed);
    if (!safe.verdict) {
      return NextResponse.json(
        { error: "The delulu check came back empty. Retry, or switch to a different model." },
        { status: 502 }
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "Delulu check failed.";
    ...
    return NextResponse.json({ error: message || "Delulu check failed. Try again." }, { status: 500 });
  }
}