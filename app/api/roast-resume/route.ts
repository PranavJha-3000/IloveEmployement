...
import { extractJsonFromLlmResponse, validateRoastResult } from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildRoastPrompt({
    resumeText,
  ...
  try {
    ...
    const safe = validateRoastResult(parsed);
    if (!safe.verdict) {
      return NextResponse.json(
        { error: "The roast came back empty. Retry, or switch to a different model." },
        { status: 502 }
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "Roast failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The roast timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 }
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "Roast failed. Try again." }, { status: 500 });
  }
}