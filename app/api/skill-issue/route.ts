...
import { validateSkillIssueResult } from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  const result = await standardAIHandler(body, {
    ...
    validateOutput: (out) => validateSkillIssueResult(out),
    isEmpty: (r) => !r.primaryIssue,
    emptyMessage: "The skill issue analysis came back empty. Retry, or switch to a different model.",
    temperature: 0.6,
  });
