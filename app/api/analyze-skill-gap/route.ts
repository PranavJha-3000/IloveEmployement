...
export async function POST(req: NextRequest) {
  ...
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. The gaps are only meaningful against a target." },
      { status: 400 },
    );
  }
  ...
  if (linkedin && linkedin.length > MAX_LINKEDIN_LENGTH) {
    return NextResponse.json(
      { error: `LinkedIn content is too long (${linkedin.length.toLocaleString()} chars). Max is ${MAX_LINKEDIN_LENGTH.toLocaleString()} - paste the key sections.` },
      { status: 400 },
    );
  }
  if (github && github.length > MAX_GITHUB_LENGTH) {
    return NextResponse.json(
      { error: `GitHub content is too long (${github.length.toLocaleString()} chars). Max is ${MAX_GITHUB_LENGTH.toLocaleString()} - paste the profile README or top repos.` },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const safe = validateSkillGapResult(parsed);

    if (safe.matrix.length === 0 && safe.biggestGaps.length === 0) {
      return NextResponse.json(
        { error: "The analysis came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    ...
    return NextResponse.json(
      { error: message || "Skill gap analysis failed. Try again." },
      { status: 500 },
    );
  }
}