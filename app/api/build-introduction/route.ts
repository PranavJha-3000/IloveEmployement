...
import type { TellMeRequestBody } from "@/lib/types";
import { createModel } from "@/lib/ai";
import { buildTellMePrompt } from "@/lib/prompts";
import {
  extractJsonFromLlmResponse,
  validateTellMeResult,
} from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  if (!resumeText?.trim()) {
    return NextResponse.json(
      { error: "Resume is required. The answer is built from your actual background." },
      { status: 400 },
    );
  }
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. 'Why this role makes sense' needs the role." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const safe = validateTellMeResult(parsed);

    if (!safe.thirtySeconds && !safe.sixtySeconds && !safe.ninetySeconds) {
      return NextResponse.json(
    ...
    const rawMessage =
      err instanceof Error ? err.message : "Introduction generation failed.";
    ...
    return NextResponse.json(
      { error: message || "Introduction generation failed. Try again." },
      { status: 500 },
    );
  }
}