...
export async function POST(req: NextRequest) {
  ...
  const { config, resumeText, jobDescription, desperationLevel, linkedinUrl, githubUrl } = body;
  ...
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. Paste the JD you are targeting." },
      { status: 400 }
    );
  }
  ...
  // ─── Profile fetching (best-effort, never blocks the rewrite) ────────────
  let linkedinContext = "";
  let githubContext = "";
  ...
  const { system, user } = buildRewritePrompt({
    resumeText,
    jobDescription,
    desperationLevel: Number.isFinite(desperationLevel) ? desperationLevel : 2,
  });
  ...
  try {
    ...
    const safe = validateRewriteResult(parsed);
    if (safe.optimizedResume.length === 0) {
      return NextResponse.json(
        { error: "The rewrite came back empty. Retry, or switch to a different model." },
        { status: 502 }
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "Rewrite failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The rewrite timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 }
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "Rewrite failed. Try again." }, { status: 500 });
  }
}