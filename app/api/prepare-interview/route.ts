...
export async function POST(req: NextRequest) {
  ...
  if (!resumeText?.trim()) {
    return NextResponse.json(
      { error: "Resume is required. Questions are generated from your actual claims." },
      { status: 400 },
    );
  }
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. The interview prep targets a specific job." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const safe = validateInterviewPrepResult(parsed);

    if (safe.questions.length === 0) {
      return NextResponse.json(
        { error: "The prep sheet came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    ...
    return NextResponse.json(
      { error: message || "Interview prep generation failed. Try again." },
      { status: 500 },
    );
  }
}