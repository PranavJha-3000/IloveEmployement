...
import type { AtsCheckRequestBody } from "@/lib/types";
import { createModel } from "@/lib/ai";
import { buildAtsPrompt } from "@/lib/prompts";
import { extractJsonFromLlmResponse, validateAtsCheckResult } from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildAtsPrompt({
    resumeText,
  ...
  try {
    ...
    const safe = validateAtsCheckResult(parsed);
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "ATS check failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The ATS check timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 }
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "ATS check failed. Try again." }, { status: 500 });
  }
}