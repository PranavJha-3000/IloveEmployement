...
export async function POST(req: NextRequest) {
  ...
  const { config, resumeText, jobDescription, linkedin, github } = body;
  ...
  if (!resumeText?.trim()) {
    return NextResponse.json(
      { error: "Resume is required. Weaknesses are found in the resume/JD mismatch." },
      { status: 400 },
    );
  }
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. Gaps only exist against a target." },
      { status: 400 },
    );
  }
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildWeaknessPrompt({
  ...
  try {
    ...
    const safe = validateWeaknessResult(parsed);

    if (safe.weaknesses.length === 0) {
      return NextResponse.json(
        { error: "The detection came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    ...
    const rawMessage =
      err instanceof Error ? err.message : "Weakness detection failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The detection timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json(
      { error: message || "Weakness detection failed. Try again." },
      { status: 500 },
    );
  }
}