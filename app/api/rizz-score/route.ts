...
import type { RizzScoreRequestBody } from "@/lib/types";
import { createModel } from "@/lib/ai";
import { buildRizzScorePrompt } from "@/lib/prompts";
import {
...
export async function POST(req: NextRequest) {
  ...
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. Paste the JD to measure alignment." },
      { status: 400 },
    );
  }
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildRizzScorePrompt({
    resumeText,
    jobDescription,
  ...
  try {
    ...
    const safe = validateRizzScoreResult(parsed);
    if (!safe.verdict) {
      return NextResponse.json(
        { error: "The rizz score came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "Rizz score calculation failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The rizz score timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "Rizz score calculation failed. Try again." }, { status: 500 });
  }
}
