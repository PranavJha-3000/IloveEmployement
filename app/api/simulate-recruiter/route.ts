...
import {
  ...
  validateRecruiterSimulationResult,
} from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  const { config, resumeText, jobDescription, role, company } = body;
  ...
  if (!resumeText?.trim()) {
    return NextResponse.json(
      { error: "Resume is required. The recruiter needs something to scan." },
      { status: 400 },
    );
  }
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. The scan is against a specific job." },
      { status: 400 },
    );
  }
  ...
  if (company && company.length > MAX_COMPANY_LENGTH) {
    return NextResponse.json({ error: `Company is too long. Max ${MAX_COMPANY_LENGTH} characters.` }, { status: 400 });
  }
  ...
  try {
    ...
    const safe = validateRecruiterSimulationResult(parsed);

    if (safe.timeline.length === 0) {
      return NextResponse.json(
        { error: "The simulation came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    ...
    const rawMessage =
      err instanceof Error ? err.message : "Recruiter simulation failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The simulation timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json(
      { error: message || "Recruiter simulation failed. Try again." },
      { status: 500 },
    );
  }
}