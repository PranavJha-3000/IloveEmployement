...
import { validateAtsCheckResult } from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  const result = await standardAIHandler(body, {
    ...
    validateOutput: (out) => validateAtsCheckResult(out),
    isEmpty: (r) => r.atsScore === 0 && r.formatChecks.length === 0,
    emptyMessage: "The ATS check came back empty. Retry, or switch to a different model.",
    temperature: 0.4,
  });
