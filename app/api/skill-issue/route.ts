...
import type { SkillIssueRequestBody } from "@/lib/types";
import { createModel } from "@/lib/ai";
import { buildSkillIssuePrompt } from "@/lib/prompts";
import {
...
export async function POST(req: NextRequest) {
  let body: SkillIssueRequestBody;
  try {
    body = await req.json();
  } catch {
  ...
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. Paste the JD to diagnose your skill issue." },
      { status: 400 },
    );
  }
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildSkillIssuePrompt({
    resumeText,
    jobDescription,
  ...
  try {
    ...
    const safe = validateSkillIssueResult(parsed);
    if (!safe.primaryIssue) {
      return NextResponse.json(
        { error: "The diagnosis came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "Skill issue diagnosis failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The diagnosis timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "Skill issue diagnosis failed. Try again." }, { status: 500 });
  }
}
