...
import {
  ...
  validateGitHubResumeResult,
} from "@/lib/utils";

export const maxDuration = 60;

const MAX_GITHUB_LENGTH = 10_000;
...
export async function POST(req: NextRequest) {
  ...
  const { config, githubInput, resumeText, jobDescription } = body;
  ...
  if (!githubInput?.trim()) {
    return NextResponse.json(
      { error: "GitHub input is required. Paste your profile URL or repository content." },
      { status: 400 },
    );
  }
  if (githubInput.length > MAX_GITHUB_LENGTH) {
    return NextResponse.json(
      { error: `GitHub input is too long (${githubInput.length.toLocaleString()} chars). Max is ${MAX_GITHUB_LENGTH.toLocaleString()} - paste your top repos and profile info.` },
      { status: 400 },
    );
  }
  if (resumeText && resumeText.length > MAX_RESUME_LENGTH) {
    return NextResponse.json(
      { error: `Resume is too long (${resumeText.length.toLocaleString()} chars). Max is ${MAX_RESUME_LENGTH.toLocaleString()}.` },
      { status: 400 },
    );
  }
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildGitHubResumePrompt({
    githubInput: githubInput.trim(),
  ...
  try {
    ...
    const safe = validateGitHubResumeResult(parsed);

    const hasContent =
      safe.githubSignal > 0 ||
      safe.projectsWorthShowing.length > 0 ||
      safe.claims.length > 0 ||
      safe.profileQuality.observations.length > 0;

    if (!hasContent) {
      return NextResponse.json(
        { error: "The check came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    ...
    const rawMessage =
      err instanceof Error ? err.message : "GitHub resume check failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The check timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json(
      { error: message || "GitHub resume check failed. Try again." },
      { status: 500 },
    );
  }
}