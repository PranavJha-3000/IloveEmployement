...
import { validateRizzScoreResult } from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  ...
  const result = await standardAIHandler(body, {
    ...
    validateOutput: (out) => validateRizzScoreResult(out),
    isEmpty: (r) => !r.verdict,
    emptyMessage: "The rizz score came back empty. Retry, or switch to a different model.",
    temperature: 0.6,
  });
