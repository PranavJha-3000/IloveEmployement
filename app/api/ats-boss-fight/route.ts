...
export async function POST(req: NextRequest) {
  ...
  const { config, resumeText, jobDescription } = body;
  ...
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. Paste the JD to fight the ATS boss." },
      { status: 400 },
    );
  }
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildAtsBossFightPrompt({
    resumeText,
    jobDescription,
  });
  ...
  try {
    ...
    const safe = validateAtsBossFightResult(parsed);
    if (safe.attackCategories.length === 0) {
      return NextResponse.json(
        { error: "The boss fight returned no results. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "ATS boss fight failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The boss fight timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "ATS boss fight failed. Try again." }, { status: 500 });
  }
}
