...
export async function POST(req: NextRequest) {
  ...
  const { config, resumeText, jobDescription, role, experienceLevel } = body;
  ...
  if (!resumeText?.trim()) {
    return NextResponse.json(
      { error: "Resume is required. The boss fights against your real claims." },
      { status: 400 },
    );
  }
  if (!jobDescription?.trim()) {
    return NextResponse.json(
      { error: "Job description is required. The questions target a specific job." },
      { status: 400 },
    );
  }
  ...
  if (experienceLevel && experienceLevel.length > MAX_EXPERIENCE_LENGTH) {
    return NextResponse.json(
      { error: `Experience level is too long. Max ${MAX_EXPERIENCE_LENGTH} characters.` },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const safe = validateBossFightSession(parsed);

    if (!safe.questions.some((q) => q.question)) {
      return NextResponse.json(
        { error: "The gauntlet came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    ...
    const rawMessage =
      err instanceof Error ? err.message : "Boss fight generation failed.";
    ...
    return NextResponse.json(
      { error: message || "Boss fight generation failed. Try again." },
      { status: 500 },
    );
  }
}