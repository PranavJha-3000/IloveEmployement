...
export async function POST(req: NextRequest) {
  ...
  // ─── Prompt ──────────────────────────────────────────────────────────────
  const { system, user } = buildEmploymentAuraPrompt({
    resumeText,
    jobDescription,
  ...
  try {
    ...
    const safe = validateEmploymentAuraResult(parsed);
    if (!safe.auraType) {
      return NextResponse.json(
        { error: "The aura came back empty. Retry, or switch to a different model." },
        { status: 502 },
      );
    }
    return NextResponse.json(safe);
  } catch (err: unknown) {
    const rawMessage = err instanceof Error ? err.message : "Aura calculation failed.";
    ...
    if (isTimeout) {
      return NextResponse.json(
        { error: "The aura timed out. Try a faster model (e.g. gpt-4o-mini or gemini-2.0-flash)." },
        { status: 504 },
      );
    }
    if (isModelError) {
    ...
    return NextResponse.json({ error: message || "Aura calculation failed. Try again." }, { status: 500 });
  }
}
